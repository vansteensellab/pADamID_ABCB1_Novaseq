...
main <- function(bam, gff, output_dir, basename=NULL, maxgap=0, shift=0,
    ...
    # Option 1) Count reads only when overlapping with the GATC flanks
    ...
    # # Option 2) Count all reads
    # reads.middle <- start(reads) + floor((end(reads)-start(reads))/2)
    # reads.middle <- GRanges(seqnames(reads),
    #                         IRanges(reads.middle, reads.middle))
    # total <- countOverlaps(gff.ranges, reads.middle, type = "any")
...
count_bins <- function(bins, gff.ranges) {
    ...
    tib_ovl <- as_tibble(ovl) %>%
        mutate(count = gff.ranges$count[queryHits]) %>%
        group_by(subjectHits) %>%
        dplyr::summarise(sum = sum(count)) %>% 
        arrange(subjectHits)
        
    mcols(bins)$count <- 0 
    mcols(bins)$count[tib_ovl$subjectHits] <- tib_ovl$sum
